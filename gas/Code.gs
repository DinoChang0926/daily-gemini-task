...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('2. 執行投資組合分析 (全部)', 'analyzeAllSheets')
    .addSeparator() // 分隔線
    .addItem('3. 分析當前列 (單筆)', 'analyzeCurrentRow')
    .addToUi();
}

/**
 * 功能 3: 分析當前游標所在的這一列 (單筆)
 */
function analyzeCurrentRow() {
  ...
  if (row < 2) {
    Browser.msgBox("請選擇第 2 列以後的資料");
    return;
  }

  const email = sheet.getRange("A2").getValue();
  if (!email || !email.toString().includes("@")) {
    Browser.msgBox("錯誤: A2 儲存格必須設定收件人 Email");
    return;
  }

  // 取得該列數據: [股票名稱, 股票代號, 成本價]
  // 假設資料在 B, C, D 欄
  const stockName = sheet.getRange(row, 2).getValue();
  let stockCode = sheet.getRange(row, 3).getValue();
  const cost = sheet.getRange(row, 4).getValue();

  if (!stockName) {
    Browser.msgBox("該列沒有股票名稱 (B欄)");
    return;
  }
  ...
  try {
    ...
    console.warn("讀取 Prompt 失敗，將使用預設值: " + e.message);
  }

  // 2. 自動補全代號 (如果沒有)
  if (!stockCode || stockCode.toString().trim() === "") {
    try {
      SpreadsheetApp.getActiveSpreadsheet().toast(`正在查詢 ${stockName} 代號...`);
      const result = queryStockCode(stockName);
      stockCode = result.toString().split('.')[0]; 
      if (stockCode.length >= 4) {
        sheet.getRange(row, 3).setValue(stockCode);
      }
    } catch (e) {
      Browser.msgBox(`查不到 ${stockName} 的代號: ${e.message}`);
      return;
    }
  }

  // 3. 執行分析
  SpreadsheetApp.getActiveSpreadsheet().toast(`正在分析 ${stockName} (${stockCode})...`, "Gemini AI", -1);
  ...
  try {
    ...
    const analysis = callGemini(userQuestion, customPrompt);
    ...
    MailApp.sendEmail({
      to: email,
      subject: `【個股分析】${stockName} (${stockCode}) - 分析報告`,
      htmlBody: `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
          <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">🔍 單筆個股健檢</h2>
          ${reportContent}
          <br><p style="color:#999; font-size:12px;">Generated by Gemini AI (Single Stick Mode)</p>
        </div>
      `
    });

    SpreadsheetApp.getActiveSpreadsheet().toast(`分析完成！已寄信給 ${email}`, "成功", 5);
    
  } catch (e) {
    Browser.msgBox(`分析 ${stockName} 失敗: ${e.message}`);
    console.error(e);
  }
}
